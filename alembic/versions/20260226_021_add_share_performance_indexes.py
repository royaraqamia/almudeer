...
def upgrade() -> None:
    ...
    # Index for task visibility queries - only if tasks table has deleted_at
    # Skip for SQLite if column doesn't exist
    ...
    # Index for device pairing lookups - only if table has deleted_at
    # Skip for SQLite if column doesn't exist
    if DB_TYPE == "postgresql":
        op.execute("""

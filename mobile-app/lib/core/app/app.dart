...
class _AppRootState extends State<AppRoot> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<offline.OfflineSyncService>().addListener(
        _onSyncStatusChange,
      );
  ...
  void dispose() {
    try {
      context.read<offline.OfflineSyncService>().removeListener(
        _onSyncStatusChange,
      );
    } catch (_) {}
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      if (!auth.isAuthenticated) {
        debugPrint('[AppRoot] Unauthenticated - Skipping Sync');
        return;
      }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // Using 'ar_AE' (UAE) to force Western numerals (0, 1, 2) but keep standard Arabic months (ظٹظ†ط§ظٹط±)

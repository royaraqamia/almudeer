  ...
  Future<void> _initializeProvider() async {
    ...
    if (mounted) {
      ...
      provider.onTransferCompleted = (session) {
        ...
        if (mounted) {
          AnimatedToast.success(
            context,
            'تم استلام ${session.metadata.fileName}',
          );
        }
      };
      ...
      provider.onTransferFailed = (session) {
        ...
        if (mounted) {
          AnimatedToast.error(context, 'فشل نقل ${session.metadata.fileName}');
        }
      };
    }
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'يرغب "${request.deviceName}" في الاتصال بك لمشاركة الملفات',
...
class _DevicesTab extends StatelessWidget {
  ...
  Widget _buildStatusCard(BuildContext context, TransferProvider provider) {
    ...
    String subtitle = 'اختر وضع الإرسال أو الاستقبال';

    if (provider.isScanning) {
      status = 'جاري البحث...';
      icon = SolarLinearIcons.magnifer;
      color = AppColors.primary;
      subtitle = 'يبحث عن الأجهزة القريبة';
    } else if (provider.isAdvertising) {
      status = 'وضع الاستقبال';
      icon = SolarLinearIcons.download;
      color = Colors.orange;
      subtitle = 'جهازك مرئي للأجهزة القريبة';
    } else if (provider.selectedDevice != null) {
      status = 'متصل';
      icon = SolarBoldIcons.checkCircle;
      color = AppColors.success;
      subtitle = 'متصل بـ ${provider.selectedDevice!.deviceName}';
    }
                  ...
                  : () async {
                      ...
                      if (provider.isScanning) {
                        ...
                        if (!success &&
                          ...
                          AnimatedToast.error(
                            context,
                            provider.errorMessage ?? 'فشل بدء البحث',
                          );
                        }
                      }
                    },
                  ...
                  : () async {
                      ...
                      if (provider.isAdvertising) {
                        ...
                        if (!success &&
                          ...
                          AnimatedToast.error(
                            context,
                            provider.errorMessage ?? 'فشل بدء الاستقبال',
                          );
                        }
                      }
                    },
      ...
      children: provider.missingRequirements.map((req) {
        ...
        switch (req) {
          case HardwareRequirement.locationPermission:
            title = 'إذن الموقع مطلوب للبحث عن الأجهزة';
            break;
          case HardwareRequirement.nearbyWifiPermission:
            title = 'إذن الأجهزة القريبة مطلوب في إصدار أندرويد هذا';
            break;
          case HardwareRequirement.bluetoothPermission:
            title = 'إذن البلوتوث مطلوب للمشاركة';
            break;
          case HardwareRequirement.locationService:
            title = 'خدمة الموقع (GPS) مغلقة';
  ...
  void _onDeviceSelected(BuildContext context, TransferDevice device) async {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'هل تريد الاتصال بـ ${device.deviceName}؟',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 16,
                fontFamily: 'IBM Plex Sans Arabic',
    ...
    if (shouldConnect == true && context.mounted) {
      ...
      if (success && context.mounted) {
        AnimatedToast.success(context, 'تم الاتصال بنجاح');
    ...
    if (result != null && result.files.isNotEmpty && context.mounted) {
      ...
      if (files.isNotEmpty) {
        final confirmed = await CustomDialog.show<bool>(
          context,
          title: 'تأكيد الإرسال',
          message: 'هل تريد إرسال ${files.length} ملفات؟',
        ...
        if (confirmed == true && context.mounted) {
          ...
          if (ids.isNotEmpty && context.mounted) {
            AnimatedToast.success(
              context,
              'تمت إضافة ${files.length} ملف إلى قائمة الانتظار',
            );
...
class _TransferCard extends StatelessWidget {
  ...
  String _formatSpeed(double speed) {
    if (speed < 0.1) return 'جاري الاتصال...';

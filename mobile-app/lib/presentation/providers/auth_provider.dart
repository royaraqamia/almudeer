...
import '../../core/services/security_event_service.dart';
...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    SecurityEventService().eventStream.listen((event) {
      if (event == SecurityEvent.accountDisabled) {
        ...
        logout(reason: 'تم تعطيل الحساب أو انتهاء الاشتراك');
      }
    });
    ...
    try {
      ...
      if (prefs.getBool('force_logout_required') == true) {
        ...
        await logout(reason: 'تم تعطيل الحساب أو انتهاء الاشتراك');
        return;
      }
  ...
  Future<bool> login(String licenseKey) async {
    ...
    if (isRateLimited) {
      _errorMessage =
          'لحماية حسابك، يرجى الانتظار $remainingLockoutMinutes دقيقة قبل المحاولة مجددًا';
    ...
    try {
      ...
      if (result.valid) {
        ...
        if (result.retryAfterSeconds != null && result.retryAfterSeconds! > 0) {
          ...
          _errorMessage = minutes > 0
              ? 'تم حظر الحساب مؤقتًا. حاول مرة أخرى بعد $minutes دقائق'
              : 'تم حظر الحساب مؤقتًا. حاول مرة أخرى بعد ${result.retryAfterSeconds} ثانية';
      ...
      _incrementAttempt();
      _errorMessage =
          'تعذر الاتصال بالخادم. تأكد من اتصالك بالإنترنت وحاول مجددًا ($e)';
      _state = AuthState.error;
      notifyListeners();
      return false;
    }
  ...
  Future<bool> addAccount(String licenseKey) async {
    ...
    final normalizedKey = licenseKey.toUpperCase().trim();
    if (!validateLicenseFormat(normalizedKey)) {
      _errorMessage = 'تنسيق المفتاح غير صحيح';
      notifyListeners();
      return false;
    }
    ...
    try {
      ...
      if (result.valid) {
        ...
        _errorMessage = result.error ?? 'المفتاح غير صالح';
        notifyListeners();
        return false;
      }
    } catch (e) {
      _errorMessage = 'حدث خطأ أثناء إضافة الحساب: $e';
      notifyListeners();
      return false;
    }
  }
  ...
  Future<void> switchAccount(UserInfo user) async {
    ...
    if (user.licenseKey == null) {
      _errorMessage = 'بيانات الحساب غير مكتملة';
      notifyListeners();
      return;
    }
    ...
    try {
      ...
      _errorMessage = 'فشل التبديل إلى الحساب';
      notifyListeners();
    } finally {
      _isSwitching = false;
    }

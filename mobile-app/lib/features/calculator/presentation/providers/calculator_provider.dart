...
class CalculatorProvider extends ChangeNotifier {
  ...
  void append(String value) {
    ...
    if (_expression.isEmpty) {
      ...
      if (isOperator(value) &&
        ...
        // Allow negative sign after * / أ— أ·
        if (value == '-' &&
            (_expression.endsWith('*') ||
                _expression.endsWith('/') ||
                _expression.endsWith('أ—') ||
                _expression.endsWith('أ·') ||
                _expression.endsWith('^'))) {
          _expression += value;
        } else {
  ...
  String? _validateScientificFunctions(String expr) {
    ...
    if (sqrtNegative.hasMatch(expr)) {
      return 'ط¬ط°ط± طھط±ط¨ظٹط¹ظٹ ظ„ط³ط§ظ„ط¨'; // Square root of negative
    }
    ...
    if (logZeroOrNegative.hasMatch(expr)) {
      return 'ظ„ظˆط؛ط§ط±ظٹطھظ… طµظپط± ط£ظˆ ط³ط§ظ„ط¨'; // Log of zero or negative
    }

    // Check for ln(0) or ln of negative numbers
    final lnZeroOrNegative = RegExp(r'ln\s*\(\s*(-?0|-[^0-9])');
    if (lnZeroOrNegative.hasMatch(expr)) {
      return 'ظ„ظˆط؛ط§ط±ظٹطھظ… ط·ط¨ظٹط¹ظٹ ظ„طµظپط± ط£ظˆ ط³ط§ظ„ط¨'; // Natural log of zero or negative
    }
  ...
  Future<void> evaluate() async {
    ...
    if (!_areParenthesesBalanced(_expression)) {
      _result = 'طھط¹ط¨ظٹط± ط؛ظٹط± طµط­ظٹط­'; // Invalid expression
      _expression = '';
      notifyListeners();
      return;
    }
    ...
    try {
      ...
      finalExpression = finalExpression.replaceAll('أ—', '*');
      finalExpression = finalExpression.replaceAll('أ·', '/');
      ...
      // Case 3: num أ— val% => num * (val/100)
      // Case 4: num أ· val% => num / (val/100)
      // Case 5: else num% => (num * 0.01)

      // Handle percentage with any operator (+, -, *, /, أ—, أ·)
      finalExpression = finalExpression.replaceAllMapped(
        RegExp(r'(\d+\.?\d*)\s*([+\-*/أ—أ·])\s*(-?\d+\.?\d*)%'),
        (match) {
          final base = match.group(1);
          final op = match.group(2);
          final percentage = match.group(3);
          // Normalize أ— and أ· to * and / for calculation
          final normalizedOp = op == 'أ—' ? '*' : (op == 'أ·' ? '/' : op);
      ...
      if (!eval.isFinite) {
        if (eval.isInfinite) {
          _result = 'ط؛ظٹط± ظ…ط¹ط±ظ‘ظپ'; // Undefined (infinity)
        } else {
          _result = 'ط®ط·ط£'; // Error (NaN)
        }
        _expression = '';
        notifyListeners();
        return;
      ...
      if (e is FormatException) {
        _result = 'طµظٹط؛ط© ط؛ظٹط± طµط­ظٹط­ط©'; // Invalid format
      } else if (e.toString().contains('parse')) {
        _result = 'طھط¹ط¨ظٹط± ط؛ظٹط± طµط­ظٹط­'; // Invalid expression
      } else {
        _result = 'ط®ط·ط£'; // Generic error
      }

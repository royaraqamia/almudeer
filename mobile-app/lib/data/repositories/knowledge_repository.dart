...
class KnowledgeRepository {
  ...
  Future<void> addKnowledgeDocument(String text) async {
    ...
    if (text.trim().isEmpty) {
      throw ArgumentError('النص لا يمكن أن يكون فارغاً');
    }
    ...
    if (text.length > KnowledgeBaseConstants.maxTextLength) {
      throw ArgumentError('النص طويل جداً (الحد الأقصى ${KnowledgeBaseConstants.maxTextLength} حرف)');
    }
  ...
  Future<void> uploadKnowledgeFile(
    ...
    if (!KnowledgeBaseConstants.allowedFileExtensions.contains(fileExtension)) {
      throw ArgumentError(
        'نوع الملف غير مدعوم. الأنواع المدعومة: ${KnowledgeBaseConstants.allowedFileExtensions.join(', ')}'
      );
    }

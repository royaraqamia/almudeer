...
/// Singleton guard for AudioService.init â€” must only be called once
...
/// Safe for concurrent callers â€” uses a Completer to serialize init.
Future<AlMudeerAudioHandler> getOrInitAudioHandler() async {
  ...
  // We are the first caller â€” do the init
  ...
  try {
    final handler = await AudioService.init(
      ...
      config: const AudioServiceConfig(
        ...
        androidNotificationChannelName: 'طھط´ط؛ظٹظ„ ط§ظ„طµظˆطھ',
...
class AudioPlayerProvider extends ChangeNotifier {
  ...
  AudioPlayerProvider() {
    // Lazy init â€” first call to playMessage will trigger initialization
  }
  ...
  Future<void> playFromPath(String filePath) async {
    ...
    try {
      ...
      _handler!.mediaItem.add(
        const MediaItem(
          id: 'preview',
          title: 'ظ…ط¹ط§ظٹظ†ط© ط§ظ„طھظژظ‘ط³ط¬ظٹظ„',
          album: 'ط§ظ„ظ…ط¯ظٹط±',
        ),
      );
  ...
  void dispose() {
    ...
    // Do NOT dispose _globalHandler â€” it outlives the provider and is app-scoped
    super.dispose();
  }

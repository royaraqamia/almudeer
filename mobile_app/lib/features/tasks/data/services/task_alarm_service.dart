...
class TaskAlarmService {
  ...
  Future<void> scheduleAlarm(TaskModel task) async {
    ...
    try {
      await _localNotifications.zonedSchedule(
        id: notificationId,
        title: 'âڈ° طھط°ظƒظٹط± ظ…ظ‡ظ…ظ‘ظژط©',
        body: '${task.title}\n${task.description ?? ''}',
      ...
      try {
        await _localNotifications.zonedSchedule(
          id: notificationId,
          title: 'âڈ° طھط°ظƒظٹط± ظ…ظ‡ظ…ظژظ‘ط©',
          body: '${task.title}\n${task.description ?? ''}',
          scheduledDate: scheduledDate,
          notificationDetails: details,
  ...
  static Future<void> showRingingOverlay(TaskModel task) async {
    final params = CallKitParams(
      id: task.id,
      nameCaller: 'طھظ†ط¨ظٹظ‡ ظ…ظ‡ظ…ظ‘ظژط©',
      appName: 'Al-Mudeer',
      handle: task.title,
      type: 0, // 0 for Audio, 1 for Video
      duration: 30000,
      textAccept: 'ظپطھط­ ط§ظ„ظ…ظ‡ظ…ظ‘ظژط©',
      textDecline: 'ط¥ط؛ظ„ط§ظ‚',

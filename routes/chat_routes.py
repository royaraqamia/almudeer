...
# --- Internal Background Tasks Implementation ---


async def send_approved_message(outbox_id: int, license_id: int):
    """
    Wrapper for services.message_sender.send_outbox_message.
    
    Kept for backward compatibility with existing background_tasks.add_task() calls.
    """
    from services.message_sender import send_outbox_message
    return await send_outbox_message(outbox_id, license_id)

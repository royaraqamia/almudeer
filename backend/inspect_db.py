...
async def inspect_telegram(conn, f):
    ...
    f.write("\n--- Active Sessions ---\n")
    for s in sessions:
        # SECURITY: Phone numbers are PII - this is a dev-only debug script
        # Do not use in production or share output files
        f.write(f"License: {s['license_key_id']}, User ID: {s['user_id']}, Phone: [REDACTED]\n")
